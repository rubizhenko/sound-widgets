  ...
  return {
    init: function init() {
      ...
      if (isNotExecuted && queue.length) {
        ...
        audioTags.on("error", function() {
          errorSoundsCount++;
          if(errorSoundsCount === audioTags.length){
            _this.showNext();
          }
        });
